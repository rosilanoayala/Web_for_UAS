(function() {
  // Canvas global (sama seperti free.js)
  const globalCanvas = document.createElement('canvas');
  globalCanvas.id = 'emberCanvas';
  document.body.prepend(globalCanvas);
  const gCtx = globalCanvas.getContext('2d');
  let width = window.innerWidth, height = window.innerHeight;
  function resize() { width = window.innerWidth; height = window.innerHeight; globalCanvas.width = width; globalCanvas.height = height; }
  ...
  function animate() { gCtx.clearRect(0,0,width,height); particles.forEach(p=>{p.update();p.draw();}); requestAnimationFrame(animate); }
  animate();
  ...
  if (canvas) {
    ...
    function resizeCanvas() {
      ...
      canvas.width = container.clientWidth * 0.9 || 800; canvas.height = 90;
    }
    ...
    class FooterEmber {
      ...
      reset() {
        ...
        this.speedX = (Math.random() - 0.5) * 0.7; this.life = 1.0;
      }
      update() {
    ...
    function animateFooter() {
      ...
      drawBase(); embers.forEach(e => { e.update(); e.draw(); });
  ...
  if (audio && sndCtrl) {
    audio.volume = 0.5; let muted = false;
    function tryPlay() {
      audio.play().then(() => {
    ...
    tryPlay();
    document.addEventListener('click', function first() { if(muted) tryPlay(); }, { once: true });
  ...
  // Countdown Timer (contoh target: 26 April 2026 20:00 WIB)
  function updateCountdown() {
    const target = new Date('2026-04-26T20:00:00+07:00').getTime();
