...
const LEADERBOARD_KEY = 'quiz_js_leaderboard';
...
function selectRandomQuestions(n) {
    return shuffleArray([...questionBankJs]).slice(0, n);
}
...
function resetLeaderboard() {
    if (confirm('Hapus semua data peringkat JavaScript? Tindakan ini tidak bisa dibatalkan.')) {
        localStorage.removeItem(LEADERBOARD_KEY);
        renderLeaderboard();
        alert('Peringkat JavaScript telah direset.');
    }
}
