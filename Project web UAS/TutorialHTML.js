...
function applyImage() {
  ...
  if (position === "center") { img.style.display = "block"; img.style.margin = "20px auto"; }
  else if (position === "left") { img.style.display = "block"; img.style.margin = "20px 0"; }
  else if (position === "right") { img.style.display = "block"; img.style.margin = "20px 0 20px auto"; }
...
function initSidebarAndCollapse() {
  ...
  sections.forEach((section, index) => {
    ...
    while (next) { childrenAfterH2.push(next); next = next.nextSibling; }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let isMoved = false, lastScrollY = window.scrollY, isHidden = false;
  const isLoggedIn = sessionStorage.getItem("isLoggedIn") === "true";
  ...
  window.addEventListener("scroll", function () {
    ...
    if (currentScroll > 100 && currentScroll > lastScrollY && !isHidden) {
